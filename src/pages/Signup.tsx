...
export default function Signup({ onLogin }: { onLogin: (token: string, user: any) => void }) {
  ...
  return (
    <div className="max-w-sm mx-auto mt-16">
      ...
      <p className="text-sm text-gray-500 mb-6">Join ShimbaData connect to manage API keys and buy credits.</p>
      <form onSubmit={submit} className="space-y-3">
        ...
        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="Email"
          className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-gray-900" />
        <input type="password" value={pass} onChange={(e) => setPass(e.target.value)} required minLength={6} placeholder="Password (6+ characters)"
          className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-gray-900" />
      ...
      <p className="text-sm text-gray-500 mt-5 text-center">
        Have an account? <Link to="/login" className="text-gray-900 font-medium hover:underline">Log in</Link>
      </p>
    </div>
  );
}
