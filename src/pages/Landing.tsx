...
export default function Landing() {
  ...
  async function genKey(e: React.FormEvent) {
    ...
    const r = await fetch(`${API}/keys`, {
      ...
      body: JSON.stringify({ name: keyName, email: keyEmail }),
    });
    ...
    <div>
      <section className="bg-gradient-to-br from-[#0a2540] to-[#13315c] text-white">
        <div className="max-w-5xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-lg text-gray-300 max-w-2xl mx-auto mb-8">
            Free, open API for ECZ past papers, subjects, and real exam questions.
            Build apps, bots, and study tools on top of {stats?.subjects ?? "—"} subjects and {stats?.questions ?? "—"} questions.
          </p>
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            <Link to="/#keys" className="border border-white/30 text-white px-6 py-3 rounded-lg hover:bg-white/10 transition">Get a free key</Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white border-y">
        <div className="max-w-5xl mx-auto px-4 py-12">
          <h2 className="text-2xl font-bold text-center text-[#0a2540] mb-8">Simple pricing</h2>
          <div className="grid sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
            <div className="border-2 border-gray-200 rounded-xl p-6">
              ...
              <Link to="/#keys" className="mt-6 block text-center bg-gray-100 text-[#0a2540] font-semibold py-2 rounded-lg hover:bg-gray-200">Start free</Link>
            </div>
            <div className="border-2 border-yellow-400 rounded-xl p-6 relative">
              <span className="absolute -top-3 right-4 bg-yellow-400 text-[#0a2540] text-xs font-bold px-2 py-1 rounded">POPULAR</span>
              <h3 className="font-bold text-lg">Pro</h3>
              <p className="text-3xl font-extrabold mt-2">$0<span className="text-base font-normal text-gray-500">/mo</span></p>
              <p className="text-xs text-gray-500 mb-2">free while in beta — invoice later</p>
              <ul className="mt-4 space-y-2 text-sm text-gray-700">
              ...
              <Link to="/#keys" className="mt-6 block text-center bg-yellow-400 text-[#0a2540] font-semibold py-2 rounded-lg hover:bg-yellow-300">Get a key</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="keys" className="max-w-2xl mx-auto px-4 py-12">
        <h2 className="text-2xl font-bold text-center text-[#0a2540] mb-6">Get your free Pro key</h2>
        <form onSubmit={genKey} className="bg-white border rounded-xl p-6 space-y-4">
          <div>
            ...
            <input value={keyName} onChange={(e) => setKeyName(e.target.value)} required
              className="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-yellow-400 outline-none" placeholder="Your name or project name" />
          </div>
          <div>
          ...
          <button className="w-full bg-[#0a2540] text-white font-semibold py-2.5 rounded-lg hover:bg-[#13315c] transition">Generate key</button>
          {newKey && (
